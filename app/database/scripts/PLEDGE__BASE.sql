...
cte_pledge_types as (
 ...
 ,[PLEDGE_CATEGORY] = IIF(SOURCECODE = 'TLCSPON', @TLC,
    IIF(SOURCECODE IN ('SACTIONP','ACTCRA','ACTCRM'), @ACTION,
    IIF(SOURCECODE = 'SPERSON', @PSP,
    IIF(SOURCECODE IN ('SCOUNTRY','SPNEPAL','SPNG', 'SNIGER', 'SINDIA', 'STHAIL', 'SNIGERIA', 'SMYANM', 'SETIMOR', 'SNIGERIA', 'SCHINA', 'SCONGO'), @CSP,
    IIF(SOURCECODE LIKE '%SCURE%' OR SOURCECODE IN ('CUREONE1', 'CUREONE2'), @CUREONE, @AMB)
    ))))
  FROM TBL_SOURCECODE
...
, cte_pledge_sourcetype as (
  SELECT cps.*, TBL_SOURCECODE.SOURCETYPE
...
, cte_legit_pledge as (
  SELECT P1.*
    ...
    ,[SPONSORSHIP1_COLOR]=IIF(PT.PLEDGE_CATEGORY = @TLC, '#007dc3',
      IIF(PT.PLEDGE_CATEGORY = @ACTION, '#fdbb30',
        IIF(PT.PLEDGE_CATEGORY = @PSP, '#a560e8',
          IIF(PT.PLEDGE_CATEGORY = @CSP, '#4dc47d',
            IIF(PT.PLEDGE_CATEGORY= @CUREONE, '#e64b50', '#b0a696')
          )
        )
      )
    )
    ,[SPONSORSHIP2]=IIF(PT.PLEDGE_CATEGORY = @TLC, PT.PLEDGE_CATEGORY,
      IIF(PT.PLEDGE_CATEGORY = @ACTION, PT.PLEDGE_CATEGORY,
        IIF(PT.PLEDGE_CATEGORY = @PSP, CONCAT(PT.PLEDGE_CATEGORY,': ', UPPER(P2.FIRST_DESTINATION2)),
          IIF(PT.PLEDGE_CATEGORY = @CSP, CONCAT(PT.PLEDGE_CATEGORY,': ', UPPER(P2.FIRST_DESTINATION1)),
