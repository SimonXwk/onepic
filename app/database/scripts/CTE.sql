...
-- ----------------------------------------------------------------------------------------------------
, cte_fishing1_payment_date AS (
  SELECT
    B1.SERIALNUMBER
    , YEAR(@FISHINGPOOL_DATE1) + CASE WHEN MONTH(@FISHINGPOOL_DATE1) < 7 THEN 0 ELSE 1 END AS [ANCHOR_FY]
    , MIN(B2.DATEOFPAYMENT) AS [FISHING_FIRST_DATEOFPAYMENT]
    , MAX(B2.DATEOFPAYMENT) AS [FISHING_LAST_DATEOFPAYMENT]
    , YEAR(MIN(B2.DATEOFPAYMENT)) + CASE WHEN MONTH(MIN(B2.DATEOFPAYMENT)) < 7 THEN 0 ELSE 1 END AS [FISHING_FIRST_DATEOFPAYMENT_FY]
    , YEAR(MAX(B2.DATEOFPAYMENT)) + CASE WHEN MONTH(MAX(B2.DATEOFPAYMENT)) < 7 THEN 0 ELSE 1 END AS [FISHING_LAST_DATEOFPAYMENT_FY]
    , CASE WHEN MIN(
  ...
  WHERE
    (B2.REVERSED IS NULL OR NOT(B2.REVERSED IN (1, -1, 2))) AND (B4.STAGE ='Batch Approved')
     AND B2.DATEOFPAYMENT <= @FISHINGPOOL_DATE1
  GROUP BY B1.SERIALNUMBER
)
-- ----------------------------------------------------------------------------------------------------
, cte_fishing2_payment_date AS (
  SELECT
    B1.SERIALNUMBER
  ...
  WHERE
    (B2.REVERSED IS NULL OR NOT(B2.REVERSED IN (1, -1, 2))) AND (B4.STAGE ='Batch Approved')
     AND B2.DATEOFPAYMENT <= @FISHINGPOOL_DATE2
  GROUP BY B1.SERIALNUMBER
)
-- ----------------------------------------------------------------------------------------------------
/*<BASE_QUERY>*/
