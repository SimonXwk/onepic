-- ----------------------------------------------------------------------------------------------------
DECLARE
    @PAYMENT_DATE1 AS VARCHAR (10) = /*<PAYMENT_DATE1>*/DATEFROMPARTS(YEAR(CURRENT_TIMESTAMP) + CASE WHEN MONTH(CURRENT_TIMESTAMP)>7 THEN 0 ELSE -1 END,7,1)/*</PAYMENT_DATE1>*/
  , @PAYMENT_DATE2 AS VARCHAR (10) = /*<PAYMENT_DATE2>*/DATEFROMPARTS(YEAR(CURRENT_TIMESTAMP) + CASE WHEN MONTH(CURRENT_TIMESTAMP)>7 THEN 1 ELSE 0 END,6,30)/*</PAYMENT_DATE2>*/
...
, cte_generation_definition AS (
  ...
  ) AS generation (Y1, Y2, GENERATION, GENERATION_ABBREVATION)
)
-- ----------------------------------------------------------------------------------------------------
, cte_contacts AS (
  SELECT RTRIM(C1.SERIALNUMBER) AS [SN]
    ,C1.CREATED
    ,CONCAT(
    ...
    , CASE WHEN C1.CONTACTTYPE='Organisation' THEN @NA ELSE CASE WHEN RTRIM(C1.GENDER)='' OR C1.GENDER IS NULL THEN @BLANK ELSE RTRIM(C1.GENDER) END END AS [GENDER]
    , C1.DATEOFBIRTH
    , (DATEDIFF(day, C1.DATEOFBIRTH, CURRENT_TIMESTAMP)+1)/365 AS [AGE]
    , C3.GENERATION, C3.GENERATION_ABBREVATION
    ...
    , CASE WHEN RTRIM(C1.MOBILENUMBER) = '' OR C1.MOBILENUMBER IS NULL THEN NULL ELSE RTRIM(C1.MOBILENUMBER) END AS [MOBILENUMBER]
    , CASE WHEN RTRIM(C1.DAYTELEPHONE) = '' OR C1.DAYTELEPHONE IS NULL THEN NULL ELSE RTRIM(C1.DAYTELEPHONE) END AS [DAYTELEPHONE]
    , CASE WHEN RTRIM(C1.EVENINGTELEPHONE) = '' OR C1.EVENINGTELEPHONE IS NULL THEN NULL ELSE RTRIM(C1.EVENINGTELEPHONE) END AS [EVENINGTELEPHONE]
    , CASE WHEN RTRIM(C1.FAXNUMBER) = '' OR C1.FAXNUMBER IS NULL THEN NULL ELSE RTRIM(C1.FAXNUMBER) END AS [FAXNUMBER]
    , CASE WHEN RTRIM(C1.EMAILADDRESS) = '' OR C1.EMAILADDRESS IS NULL THEN NULL ELSE RTRIM(C1.EMAILADDRESS) END AS [EMAILADDRESS]
  ...
  FROM
    ...
    LEFT JOIN cte_generation_definition C3 ON (C1.DATEOFBIRTH >= DATEFROMPARTS(C3.Y1, 1, 1) AND C1.DATEOFBIRTH < DATEFROMPARTS(C3.Y2+1, 1,1) )
