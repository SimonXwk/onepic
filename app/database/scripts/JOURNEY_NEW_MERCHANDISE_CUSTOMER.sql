...
--   ,[FULLNAME] = MIN(t2.PAYMENT_FULLNAME)
  ...
  ,[FIRST_CAMPAIGN] =
    CASE WHEN COUNT(DISTINCT CASE WHEN t2.DATEOFPAYMENT = t1.FIRSTDATE AND t2.CAMPAIGNCODE NOT LIKE '%Post%' AND t2.CAMPAIGNCODE NOT LIKE '%Aux%' THEN t2.CAMPAIGNCODE ELSE NULL END) = 0
    THEN MIN(CASE WHEN t2.DATEOFPAYMENT = t1.FIRSTDATE THEN t2.CAMPAIGNCODE ELSE NULL END)
    ELSE MIN(CASE WHEN t2.DATEOFPAYMENT = t1.FIRSTDATE AND t2.CAMPAIGNCODE NOT LIKE '%Post%' AND t2.CAMPAIGNCODE NOT LIKE '%Aux%' THEN t2.CAMPAIGNCODE ELSE NULL END)
    END
