...
,TOTAL_FY AS (
  SELECT
    (YEAR(B2.DATEOFPAYMENT)+IIF(MONTH(B2.DATEOFPAYMENT)<@FY1M OR @FY1M=1,0,1)) AS FY
  ...
  WHERE
    ...
    AND (YEAR(B2.DATEOFPAYMENT)+IIF(MONTH(B2.DATEOFPAYMENT)<@FY1M OR @FY1M=1,0,1)) <= ?
  GROUP BY
    (YEAR(B2.DATEOFPAYMENT)+IIF(MONTH(B2.DATEOFPAYMENT)<@FY1M OR @FY1M=1,0,1))
)
-- --------------------------------------------------------------
select
  T1.FY,T2.TOTAL
