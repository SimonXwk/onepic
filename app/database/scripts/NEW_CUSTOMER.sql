...
SELECT
  ...
  , ISSPONSORSHIP=IIF(S1.SOURCETYPE LIKE 'MERCH%SPONSORSHIP%', -1, 0)
...
SELECT SERIALNUMBER, DATEOFPAYMENT
    ,ORDERID = IIF(MANUALRECEIPTNO IS NULL OR NOT(RTRIM(MANUALRECEIPTNO) LIKE @REX_RECEIPT_PATTERN),
      SUBSTRING(NOTES,PATINDEX(@REX_RECEIPT_NOTE_PATTERN,NOTES)+LEN(@REX_RECEIPT_PREFIX)
      ,LEN(@REX_RECEIPT_PATTERN)), MANUALRECEIPTNO)
FROM TBL_BATCHITEM
WHERE
...
-- --------------------------------------------------------------
 ,cte_onboard as (
SELECT TOP 1 SERIALNUMBER, PARAMETERNAME, PARAMETERVALUE, EFFECTIVEFROM, EFFECTIVETO, PARAMETERNOTE
...
select
  ...
  ,MERCHANDISE_PLEDGE = SUM(IIF(t3.ISSPONSORSHIP = -1, t3.PAYMENTAMOUNT,0))
  ...
  ,t4.PARAMETERNAME, t4.PARAMETERVALUE, t4.EFFECTIVEFROM, t4.EFFECTIVETO, t4.PARAMETERNOTE
from
  cte_first_date t1
  ...
  left join cte_onboard t4 on (t1.SERIALNUMBER = t4.SERIALNUMBER)
where
