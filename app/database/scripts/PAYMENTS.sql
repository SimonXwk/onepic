-- ----------------------------------------------------------------------------------------------------
DECLARE @DATE1 VARCHAR (10) = /*<PAYMENT_DATE1>*/DATEFROMPARTS(YEAR(CURRENT_TIMESTAMP) + CASE WHEN MONTH(CURRENT_TIMESTAMP)>7 THEN 0 ELSE -1 END,7,1)/*</PAYMENT_DATE1>*/,
...
select t1.*
  ...
  , CASE WHEN
      YEAR(@DATE1) + CASE WHEN MONTH(@DATE1) <7 THEN 0 ELSE 1 END = YEAR(@DATE2) + CASE WHEN MONTH(@DATE2) <7 THEN 0 ELSE 1 END
      AND MONTH(@DATE1) = 7 AND DAY(@DATE1) =1
    THEN
