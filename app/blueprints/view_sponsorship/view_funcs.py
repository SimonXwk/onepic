...
def pledge_created_fys():
	data = Tq.query('LIST_FY_PLEDGE_CREATED', cached_timeout=300)
	return data.rows


def pledge_dop_fys():
	data = Tq.query('LIST_FY_DATEOFPAYMENT_SPONSORSHIP', cached_timeout=300)
	return data.rows
...
def pledges(fy=TLMA.cfy):
	update = [('FY', fy)]
	data = Tq.query('PLEDGES', updates=update, cached_timeout=10)
...
def pledge_income(fy=TLMA.cfy):
	update = [('FY', fy)]
	data = Tq.query('PLEDGE_INCOME', updates=update, cached_timeout=25)
	fys = pledge_dop_fys()
