...
def rfm_calculate_score(value, q_dict, one_is_worst=True):
    """
...
def rfm_analysis(file_name=None):
    if file_name:
        ...
        rfm['r_score'] = rfm['recency'].apply(rfm_calculate_score, q_dict=quantile['r'], one_is_worst=False)
        rfm['f_score'] = rfm['frequency'].apply(rfm_calculate_score, q_dict=quantile['f'])
        rfm['m_score'] = rfm['monetary_value'].apply(rfm_calculate_score, q_dict=quantile['m'])
